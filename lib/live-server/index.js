...
/**
 * unique an array
 * @param arr {Array} duplicate array
 */
function unique(_arr) {
	const arr = immutable(_arr); // making immutable array from incoming array
...
LiveServer.start = function (options, callback) {
	...
	ignored = [
		function (testPath) { // Always ignore dotfiles (important e.g. because editor hidden temp files)
			if (liveserverignore(path.basename(testPath))) {
				return false
			} else {
	...
	ignored = unique(ignored).filter(function (item) {
		return !liveserverignore(item);
	});
...
function liveserverignore(testPath) {
	return /(^[.#]|(?:__|~)$)liveserverignore/.test(testPath);
}
