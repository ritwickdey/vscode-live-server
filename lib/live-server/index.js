...
LiveServer.start = function (options, callback) {
	...
	var wait = options.wait === undefined ? 0 : options.wait;
	...
	server.addListener('upgrade', function (request, socket, head) {
		...
		ws.on('blockLiveEditing', function () {
			blockLiveEditing = true;
		});
	...
	LiveServer.liveEditing = function (doc, content) {
		if (blockLiveEditing) {
			var changePath = doc.document.fileName
			var cssChange = path.extname(changePath) === ".css";
			var match, injectTag = null;
			...
			const $ = cheerio.load(content.replace(new RegExp(injectTag, "i"), GET_INJECTED_CODE() + injectTag));

			clients.forEach(function (ws) {
				const wsFile = slash(ws.url).split('/');
				const changePathFile = slash(changePath).split('/');
				const _regex = /\.(js|css|html|scss|sass|jsx)$/;
				const wsFileIndex = _.findIndex(wsFile, function(o) { return o.match(_regex) })
				const changePathFileIndex = _.findIndex(changePathFile, function(o) { return o.match(_regex) })
				...
				if (ws && wsFileIndex > -1 && changePathFileIndex > -1) { // (wsFile[wsFileIndex] === changePathFile[changePathFileIndex])
					(cssChange && !fullReload) ? ws.send(JSON.stringify({
						cmd: 'refreshcss'
					})) : ws.send(JSON.stringify({
						cmd: 'liveEditing',
						content: $.html()
