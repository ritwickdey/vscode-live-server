	...
	return function (req, res, next) {
		...
		function inject(stream) {
			if (injectTag) {
				...
				let len = GET_INJECTED_CODE().length + res.getHeader('Content-Length');
...
LiveServer.start = function (options, callback) {
	...
	// let addtionalHTMLtags = options.addtionalHTMLtags || []; //dropped the feature
	let disableGlobbing = options.disableGlobbing || false;
	let onTagMissedCallback = options.onTagMissedCallback || null;
	let fullReload = options.fullReload || false;

	let staticServerHandler = staticServer(root, onTagMissedCallback);
