...
var useBrowserExtension = false;
var GET_INJECTED_CODE = () => useBrowserExtension === true ? '' : INJECTED_CODE;
 ...
 * @param wsName {string} Websocket service name (default /WSService)
 */
LiveServer.start = function (options, callback) {
	...
	var wsName = options.wsName || '/WSService';
	/*new*/
	...
	var staticRequestHandler = staticServer(root, onTagMissedCallback);
	var httpRequestHandler = function (req, res, next) {
		var hr = { handled: false, next: next };
		server.emit('http.fs.request', req, res, hr);
		if (!hr.handled)
			staticRequestHandler(req, res, next);
	};
	...
	app.use(httpRequestHandler) // Custom static server
		.use(entryPoint(httpRequestHandler, file))
		.use(serveIndex(root, {
			icons: true
		}));
	...
	// WebSocket
	var clients = [];
	var liveWebsocketClients = [];
	var onPageWSRequest = function (request, socket, head) {
		if (!request.url.endsWith("/ws")) {
			var proxyURL = url.parse(proxy[0][1]);
	...
	var onLiveWSRequest = function (request, socket, head) {
		var ws = new WebSocket(request, socket, head);
		ws.onopen = function () {
			liveWebsocketClients.push(ws);
			server.emit("ws.client.open", ws, request.headers);
		};

		ws.onmessage = function (ev) {
			server.emit("ws.client.message", ws, ev.data);
		};

		ws.onclose = function (ev) {
			liveWebsocketClients = liveWebsocketClients.filter(function (x) {
				return x !== ws;
			});
			server.emit("ws.client.close", ws, ev.code, ev.reason);
		};
		Object.defineProperties(ws, {
			'id': {
				get: function () {
					var c = this;
					var p = (this.protocol && c.protocol.length > 0) ? ('[' + c.protocol + ']') : ('');
					return c.remoteFamily + p + '://' + c.remoteAddress + ':' + c.remotePort;
				}
			},
			'remoteAddress': { get: function () { return this._stream.remoteAddress; } },
			'remoteFamily': { get: function () { return this._stream.remoteFamily; } },
			'remotePort': { get: function () { return this._stream.remotePort; } }
		});
	};

	Object.defineProperty(server, "WSClients", {
		get: function () { return liveWebsocketClients; }
	});

	(function () {
		var oclose = server.close;
		server.close = function () {
			var args = arguments;
			for (var i = 0; i < clients.length; ++i) {
				try { clients[i].close(); } catch (ex) { }
			}
			clients = [];

			for (var i = 0; i < liveWebsocketClients.length; ++i) {
				try { liveWebsocketClients[i].close(); } catch (ex) { }
			}
			liveWebsocketClients = [];
			oclose.apply(server, args);
		};
	})();

	server.addListener('upgrade', function (request, socket, head) {
		if (request.url === wsName) {
			onLiveWSRequest(request, socket, head);
		} else {
			onPageWSRequest(request, socket, head);
		}
	});
