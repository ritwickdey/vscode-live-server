...
LiveServer.start = (options, callback) => {
	...
	server.addListener('listening', (/*e*/) => {
		...
		if (LiveServer.logLevel >= 1) {
			if (serveURL === openURL)
				if (serveURLs.length === 1) console.log(('Serving "%s" at %s').green, root, serveURLs[0]);
				else console.log(('Serving "%s" at\n\t%s').green, root, serveURLs.join('\n\t'));
			else console.log(('Serving "%s" at %s (%s)').green, root, openURL, serveURL);
		}

		// Launch browser
		if (openPath !== null)
			if (typeof openPath === 'object') openPath.forEach(p => open(openURL + p, { app: browser }));
			else open(openURL + openPath, { app: browser });
	});
	...
	if (options.ignore) ignored = ignored.concat(options.ignore);
	if (options.ignorePattern) ignored.push(options.ignorePattern);
	// Setup file watcher
	...
	function handleChange (changePath) {
		...
		clients.filter(ws => ws).forEach(ws => ws.send((cssChange && !fullReload) ? 'refreshcss' : 'reload'))
	}
