...
function staticServer(root, extraInjectCandidates) {
	var isFile = false;
...
LiveServer.start = function (options, callback) {
	...
	/*new*/
	var customHtmlTagPrefer = customHtmlTagPrefer || []; 
	var disableGlobbing = options.disableGlobbing || false;
	var staticServerHandler = staticServer(root,customHtmlTagPrefer);
	...
	mount.forEach(function (mountRule) {
		...
		app.use(mountRule[0], staticServer(mountPath,staticServer));
