...
LiveServer.start = function (options, callback) {
	...
	var watchPaths = options.watch || [...roots];
	...
	var serveMaps = options.serveMaps || [];
	
	var staticServerHandlers = [];
	roots.forEach(root => {
		staticServerHandlers.push(staticServer(root, onTagMissedCallback));
	});
	...
	mount.forEach(function (mountRule) {
		...
		app.use(mountRule[0], staticServer(mountPath, onTagMissedCallback));
	...
	staticServerHandlers.forEach((staticServerHandler, i) => {
		let mapping = (serveMaps.length - 1) >= i ? '/' + serveMaps[i] : '/';
		app.use(mapping,staticServerHandler) // Custom static server
		.use(mapping,entryPoint(staticServerHandler, file))
	});
	
	app.use(serveIndex(roots[0], {
			icons: true
	}));
		...
		if (LiveServer.logLevel >= 1) {
			if (serveURL === openURL)
				if (serveURLs.length === 1) {
					console.log(("Serving \"%s\" at %s").green, roots, serveURLs[0]);
				} else {
					console.log(("Serving \"%s\" at\n\t%s").green, roots, serveURLs.join("\n\t"));
				}
			else
				console.log(("Serving \"%s\" at %s (%s)").green, roots, openURL, serveURL);
		}
